...
gen.mjs 会在输出目录随产物落一份 `player.html`(视频循环播放 + 雪碧图 canvas 翻帧 + 透明版棋盘格/白/深/品红底切换 + fps 调节)。本机起服务即可看:

```bash
node <本仓库>/preview/serve.mjs --dir <输出目录> --port 8790
# 浏览器打开 http://127.0.0.1:8790/player.html
```

注意 8787 等常用端口可能被其它本机服务占用,换个端口即可;改了产物刷新页面即见(no-store 不缓存)。
